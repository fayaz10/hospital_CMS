                    ...
                    <div class="col-xl-12 col-md-12 ">
                        ...
                        <div class="m-form__section m-form__section--first">
                            ...
                            <div class="m-form__heading ">
                                <h3 class="m-form__heading-title"> {{__('finance.incomeInfo')}}</h3>
                            </div>

                            <div class="row form-group m-form__group ">
                                <div class="col-lg-4">
                                    <label class="form-control-label">{{__('finance.incSection')}} </label>
                                    <p><strong>{{ __('finance.'.strtolower(basename($income->profitable_type, '\\'))) }}</strong>
                                    </p>
                                </div>
                                <div class="col-lg-4">
                                    <label class="form-control-label">{{__('finance.paymentDate')}}</label>
                                    <p><strong>{{ $income->payment_date }}</strong></p>
                                </div>
                                <div class="col-lg-4">
                                    <label class="form-control-label">{{__('lab.lab_expCreatedAt')}}</label>
                                    <p><strong>{{ $income->created_at->format('Y-m-d g:i A') }}</strong></p>
                                </div>
                            </div>
                            <div class="row form-group m-form__group ">
                                <div class="col-lg-4">
                                    ...
                                    <p><strong>
                                            {{ $income->amount }}
                                            {{ app()->isLocale('en') ? $income->currency->label_en : $income->currency->label_dr  }}
                                        </strong></p>
                                    </div>
                                    <div class="col-lg-4">
                                        <label class="form-control-label"> {{__('pharmacist.med_register')}} </label>
                                        <p><strong>
                                            {{ app()->isLocale('en') ? $income->registrar->name : $income->registrar->name_dr  }}
