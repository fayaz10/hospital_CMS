                                ...
                                <div class="form-group m-form__group row">
                                    <table class="table m-table m-table--head-separator-primary">
                                        <thead class="table-inverse">
                                            <tr>
                                                <th><i class="text-danger">*</i> {{__('pharmacist.med_name')}}</th>
                                                <th><i class="text-danger">*</i> {{__('pharmacist.med_milligram')}}</th>
                                                <th><i class="text-danger">*</i> {{__('pharmacist.med_company')}}</th>
                                                <th>{{__('pharmacist.med_excistNow')}}</th>
                                                <th>{{__('pharmacist.med_price')}}</th>
                                                <th><i class="text-danger">*</i> {{__('pharmacist.med_type')}}</th>
                                                <th><i class="text-danger">*</i> {{__('pharmacist.med_unit')}}</th>
                                                <th>
                                            ...
                                            <tr>
                                                ...
                                                <td style="min-width:130px">
                                                    <select class="form-control m-input" name="items[0][type_id]">
                                                        ...
                                                        <option value="{{ $type->id }}" {{ old("type_id") == $type->id ? "selected" : "" }}>
                                                            {{ app()->isLocale("en") ? $type->label_en : $type->label_dr }}
                                                        </option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td style="min-width:130px">
                                                    <select class="form-control m-input" name="items[0][unit_id]">
                                                        ...
                                                        <option value="{{ $unit->id }}" {{ old("unit_id") == $unit->id ? "selected" : "" }}> {{ app()->isLocale("en") ? $unit->label_en : $unit->label_dr }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td>
...
<script type="text/javascript">
    ...
    function addNew(e) {
        ...
        var inputsStart = '<tr>' +
            ...
            '<td> <select class="form-control m-input" name="items[' + index + '][type_id]"> @foreach(\App\Models\Pharmacist\MedicineType::all() as $type) <option value="{{$type->id}}"{{old("type_id")==$type->id ? "selected" : ""}}>{{app()->isLocale("en") ? $type->label_en : $type->label_dr}}</option> @endforeach </select> </td>' +
            '<td> <select class="form-control m-input" name="items[' + index + '][unit_id]"> @foreach(\App\Models\Pharmacist\Unit::all() as $unit) <option value="{{$unit->id}}"{{old("unit_id")==$unit->id ? "selected" : ""}}>{{app()->isLocale("en") ? $unit->label_en : $unit->label_dr}}</option> @endforeach </select> </td>';
