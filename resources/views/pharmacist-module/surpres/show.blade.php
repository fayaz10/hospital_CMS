            ...
            <div class="m-form m-form--label-align-right">
                <div class="row align-items-center">
                    ...
                    <div class="col-xl-8 order-1 order-xl-2 m--align-right">
                        ...
                        @endphp
                        @if ($status && $status->is_approved === null)
                            <a href="#"
                                class="btn m-btn btn-info m-loader m-loader--light m-loader--left">
                                <span>{{__('global.sent2Approve')}}</span> </span> </a>
                        @elseif($status && $status->is_approved === 0)
                            <a href="{{ route('surpres.sent2Approve', [$surpre->id]) }}" 
                                onclick="return confirm('{{ __('global.irreversable') }}')"
                                class="btn btn-info">
                                <span> <i class="la la-share"></i> <span>{{__('global.sent2ApproveAgain')}}</span> </span> </a>
                        @elseif($status && $status->is_approved === 1)
                            <a href="#" 
                                class="btn btn-info">
                                <span> <i class="la la-share"></i> <span>{{__('global.received')}}</span> </span> </a>
                        @else
                            <a href="{{ route('surpres.sent2Approve', [$surpre->id]) }}" 
                                onclick="return confirm('{{ __('global.irreversable') }}')"
                                class="btn btn-info">
                                <span> <i class="la la-share"></i> <span>{{__('global.sent2Approve')}}</span> </span> </a>
                        @endif

                        {{-- <a href="{{ route('surpres.refund', [$surpre->id]) }}" class="btn btn-warning"
                        ...
                        <div class="m-separator m-separator--dashed d-xl-none"></div>
                        <a href="{{ route('surpres.print', [$surpre->id]) }}" class="btn btn-secondary">
                        ...
                        <div class="m-separator m-separator--dashed d-xl-none"></div>
                        <a href="{{ route('surpres.edit', [$surpre->id]) }}" class="btn btn-secondary">
                        ...
                        <div class="m-form__section m-form__section--first">
                            ...
                            <div class="row form-group m-form__group ">
                                <div class="col-lg-3">
                                    ...
                                    <p><strong>
                                            {{ $surpre->profit->amount }}
                                            {{ app()->isLocale('en') ? $surpre->profit->currency->label_en : $surpre->profit->currency->label_dr }}
                                        </strong></p>
                                </div>
                                <div class="col-lg-3">
                                ...
                                <div class="col-lg-3">
                                    ...
                                    <p><strong>
                                            {{ app()->isLocale('en') ? $surpre->profit->registrar->name : $surpre->profit->registrar->name_dr }}
                                        </strong></p>
                                </div>
                            </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('stock-out.store') }}" id="add-form" method="post">
                <div class="modal-body">
                    {{ csrf_field() }}
                    <input type="hidden" name="prescription_id" value="{{ $surpre->id }}">
