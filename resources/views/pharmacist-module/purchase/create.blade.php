...
<!-- <link href="{{ asset('css/jexcel.css') }}" rel="stylesheet">
<link href="{{ asset('css/jsuites.css') }}" rel="stylesheet"> -->
<style type="text/css">
    /* input {
...
<div class="m-content">
    <div class="m-portlet m-portlet--last m-portlet--head-lg m-portlet--responsive-mobile" id="main_portlet">
                ...
                <div class="m-portlet__head-caption">
                    <div class="m-portlet__head-title">
                        ...
                        <h3 class="m-portlet__head-text">
                         {{__('pharmacist.med_addnewPurchase')}}
                        </h3>
                    </div>
                </div>
        ...
        <div class="m-portlet__body">
            ...
            <form action="{{ route('medicine-purchase.store') }}" id="save-form" class="m-form m-form--label-align-left m-form--state col-12" method="post">
                {{ csrf_field() }}
                <!--begin: Form Body -->
                <div class="m-portlet__body">
                    <div class="row">
                        <div class="col-xl-12 col-md-12 ">
                            ...
                            <div class="m-form__section m-form__section--first">
                                <div class="m-form__heading">
                                    <h3 class="m-form__heading-title">{{__('pharmacist.med_infonewPurchase')}}</h3>
                                </div>
                                <div class="form-group m-form__group row">
                                    <div class="col-lg-3 m-form__group-sub">
                                        <label class="form-control-label"><span class="text-danger">* </span> {{__('pharmacist.med_purchase_date')}}</label>
                                        <input type="date" required value="{{ old('purchase_date', date('Y-m-d')) }}" name="purchase_date" required class="form-control m-input" placeholder="">
                                    </div>
                                    <div class="col-lg-3 m-form__group-sub">
                                        <label class="form-control-label"><span class="text-danger">* </span> {{__('pharmacist.med_payment')}}</label>
                                        <input type="text" required value="{{ old('remitter',app()->getLocale() == 'en' ? auth()->user()->name : auth()->user()->name_dr) }}" name="remitter" , required class="form-control m-input" placeholder="">
                                    </div>
                                    <div class="col-lg-3 m-form__group-sub">
                                        <label class="form-control-label">{{__('pharmacist.med_indusCompany')}}</label>
                                        <input type="text" value="{{ old('suppliers') }}" name="suppliers" class="form-control m-input" placeholder="">
                                    </div>
                                    <div class="col-lg-3 m-form__group-sub">
                                        <label class="form-control-label"><span class="text-danger">* </span>{{__('pharmacist.med_currency')}}</label>
                                        <select class="form-control m-input" name="currency_id">
                                            @foreach(\App\Models\FinanceModule\Currency::all() as $type)
                                                <option value="{{ $type->id }}">
                                                    {{ app()->getLocale() == 'en' ? $type->label_en : $type->label_dr }} ({{ $type->symbol }})
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <table class="table m-table m-table--head-separator-primary">
                                        <thead class="table-inverse">
                                            <tr>
                                                ...
                                                <th> 48% {{__('pharmacist.med_benefit')}}</th>
...
<script type="text/javascript">
    ...
    function calculateBU(param){
        ...
        var benefitsConstant = +'{{ config("iSys.benefits.medicinePurchase", config("iSys.benefits.default")) }}';
